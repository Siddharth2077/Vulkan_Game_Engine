...
namespace vkinit {
    ...
    /// <summary>
    /// [Helper] Initializes and returns a VkCommandPoolCreateInfo struct.
    /// </summary>
    /// <param name="queueFamilyIndex">: The queue family that your command pool will be associated with. </param>
    /// <param name="createFlags">: [default value : VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT]. A bitmask of VkCommandPoolCreateFlagBits indicating usage behavior for the pool and command buffers allocated from it.</param>
    /// <param name="pNextPointer">: [default value : nullptr]. A pointer to a structure extending this structure. </param>
    /// <returns></returns>
    VkCommandPoolCreateInfo command_pool_create_info(
        uint32_t queueFamilyIndex, 
        VkCommandPoolCreateFlags createFlags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, 
        const void* pNextPointer = nullptr
    );

    /// <summary>
    /// [Helper] Initializes and returns a VkCommandBufferAllocateInfo struct.
    /// </summary>
    /// <param name="commandPool">: The command pool from which the command buffers are allocated. </param>
    /// <param name="count">: [default value : 1]. The number of command buffers to allocate from the pool. </param>
    /// <param name="commandBufferLevel">: [default value : VK_COMMAND_BUFFER_LEVEL_PRIMARY]. A VkCommandBufferLevel value specifying the command buffer level.</param>
    /// <param name="pNextPointer">: [default value : nullptr]. A pointer to a structure extending this structure. </param>
    /// <returns></returns>
